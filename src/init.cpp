...
void Shutdown()
{
    ...
    if (pwalletMain)
        bitdb.Flush(false);
    GenerateMonetaryUnits(false, NULL, 0);
#endif
    StopNode();
...
std::string HelpMessage(HelpMessageMode hmm)
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: monetaryunit.conf)") + "\n";
    if (hmm == HMM_MONETARYUNITD)
    {
#if !defined(WIN32)
    ...
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: monetaryunitd.pid)") + "\n";
...
bool InitSanityCheck(void)
{
    if(!ECC_InitSanityCheck()) {
        InitError("OpenSSL appears to lack support for elliptic curve cryptography. For more "
                  "information, visit https://en.monetaryunit.it/wiki/OpenSSL_and_EC_Libraries");
        return false;
    }
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (pwalletMain)
        GenerateMonetaryUnits(GetBoolArg("-gen", false), pwalletMain, GetArg("-genproclimit", -1));
#endif
