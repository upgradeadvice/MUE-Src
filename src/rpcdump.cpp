...
Value importwallet(const Array& params, bool fHelp)
{
    ...
    while (file.good()) {
        ...
        if (pwalletMain->HaveKey(keyid)) {
            LogPrintf("Skipping import of %s (key already present)\n", CMonetaryUnitAddress(keyid).ToString());
            continue;
        }
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    ...
    return CMonetaryUnitSecret(vchSecret).ToString();
}
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        std::string strAddr = CMonetaryUnitAddress(keyid).ToString();
        CKey key;
        if (pwalletMain->GetKey(keyid, key)) {
            if (pwalletMain->mapAddressBook.count(keyid)) {
                file << strprintf("%s %s label=%s # addr=%s\n", CMonetaryUnitSecret(key).ToString(), strTime, EncodeDumpString(pwalletMain->mapAddressBook[keyid].name), strAddr);
            } else if (setKeyPool.count(keyid)) {
                file << strprintf("%s %s reserve=1 # addr=%s\n", CMonetaryUnitSecret(key).ToString(), strTime, strAddr);
            } else {
