...
void WalletTxToJSON(const CWalletTx& wtx, Object& entry)
{
    ...
    if (wtx.nVersion >= CTransaction::TXCOMMENT_VERSION)
    {
        if (wtx.strTxComment.length() > 0)
        {
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress <monetaryunitaddress> <amount> [comment] [comment-to] [tx-comment]\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <monetaryunitaddress> [minconf=1]\n"
            "Returns the total amount received by <monetaryunitaddress> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom <fromaccount> <tomonetaryunitaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.00000001"
            + HelpRequiringPassphrase());
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <monetaryunitaddress>\n"
            "Return information about <monetaryunitaddress>.");
