...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (block.nVersion >= 116 && CBlockIndex::IsSuperMajority(116, pindex->pprev, Params().EnforceBlockUpgradeMajority())) {
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Reject block.nVersion < 115 blocks when 95% (75% on testnet) of the network has upgraded:
    if (nHeight >=1130000 && block.nVersion < 115 && CBlockIndex::IsSuperMajority(115, pindexPrev, Params().RejectBlockOutdatedMajority()))
    {
        return state.Invalid(error("%s : rejected nVersion < 115 block", __func__),
                             REJECT_OBSOLETE, "bad-version");
    }

    // Reject block.nVersion=115 blocks when 95% (75% on testnet) of the network has upgraded:
    if (block.nVersion < 116 && CBlockIndex::IsSuperMajority(116, pindexPrev, Params().RejectBlockOutdatedMajority()))
        return state.Invalid(error("%s : rejected nVersion < 116 block", __func__),
                             REJECT_OBSOLETE, "bad-version");
