// Copyright (c) 2011-2014 The MonetaryUnit developers
...
{
    CMonetaryUnitAddress addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered monetaryunit address / amount:
            ...
            scriptPubKey.SetDestination(CMonetaryUnitAddress(rcp.address.toStdString()).Get());
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, transaction.getRecipients())
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal monetaryunit:URI (monetaryunit:123...?message=example)
...
{
    QString strAddress = QString::fromStdString(CMonetaryUnitAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        mapCoins[CMonetaryUnitAddress(address).ToString().c_str()].push_back(out);
    }
}
...
{
    CTxDestination dest = CMonetaryUnitAddress(sAddress).Get();
