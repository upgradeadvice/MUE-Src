...
const int MONETARYUNIT_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString MONETARYUNIT_IPC_PREFIX("monetaryunit:");
const char* MONETARYUNIT_REQUEST_MIMETYPE = "application/monetaryunit-paymentrequest";
const char* MONETARYUNIT_PAYMENTACK_MIMETYPE = "application/monetaryunit-paymentack";
const char* MONETARYUNIT_PAYMENTACK_CONTENTTYPE = "application/monetaryunit-payment";
...
bool PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(MONETARYUNIT_IPC_PREFIX, Qt::CaseInsensitive)) // monetaryunit: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseMonetaryUnitURI(arg, &r) && !r.address.isEmpty())
            {
                CMonetaryUnitAddress address(r.address.toStdString());
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    foreach (const QString& r, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(MONETARYUNIT_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            return false;
        }
        ...
        socket->waitForBytesWritten(MONETARYUNIT_IPC_CONNECT_TIMEOUT);
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(MONETARYUNIT_IPC_PREFIX, Qt::CaseInsensitive)) // monetaryunit: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseMonetaryUnitURI(s, &recipient))
                emit receivedPaymentRequest(recipient);
            else
                emit message(tr("URI handling"),
                    tr("URI can not be parsed! This can be caused by an invalid MonetaryUnit address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    foreach(const PAIRTYPE(CScript, qint64)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CMonetaryUnitAddress(dest).ToString()));
        }
        ...
        if (txOut.IsDust(CTransaction::nMinRelayTxFee)) {
            QString msg = tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(MonetaryUnitUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second));
...
void PaymentServer::fetchRequest(const QUrl& url)
{
    ...
    netRequest.setRawHeader("Accept", MONETARYUNIT_REQUEST_MIMETYPE);
...
void PaymentServer::fetchPaymentACK(CWallet* wallet, SendCoinsRecipient recipient, QByteArray transaction)
{
    ...
    netRequest.setHeader(QNetworkRequest::ContentTypeHeader, MONETARYUNIT_PAYMENTACK_CONTENTTYPE);
    netRequest.setRawHeader("User-Agent", CLIENT_NAME.c_str());
    netRequest.setRawHeader("Accept", MONETARYUNIT_PAYMENTACK_MIMETYPE);
