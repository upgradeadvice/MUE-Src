    ...
    model(0)
{
...
#if QT_VERSION >= 0x040700
    /* Do not move this to the XML file, Qt before 4.7 will choke on it */
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a Monetaryunit address (e.g. 787JDE6UGZ1unuLrQntP3eza5qeZY22t5U)"));
#endif
    ...
    connect(clipboardMinusFeeAction, SIGNAL(triggered()), this, SLOT(coinControlClipboardMinusFee()));
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::QRK, rcp.amount), Qt::escape(rcp.label), rcp.address));
      #else
        formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::QRK, rcp.amount), rcp.label.toHtmlEscaped(), rcp.address));
      #endif
    }
    ...
    case WalletModel::AmountWithFeeExceedsBalance:
        QMessageBox::warning(this, tr("Send Coins"),
            ...
            arg(BitcoinUnits::formatWithUnit(BitcoinUnits::QRK, sendstatus.fee)),
            QMessageBox::Ok, QMessageBox::Ok);
        break;
...
// Coin Control: copy label "Minus fee" to clipboard
void SendCoinsDialog::coinControlClipboardMinusFee()
{
...
void SendCoinsDialog::coinControlFeatureChanged(bool checked)
{
    ...
    {
        model->coinControlChangeAddress("");
        model->coinControlUnSelectAll();
    }
}
...
{
    if (model)
        model->coinControlChangeAddress((state == Qt::Checked ? ui->lineEditCoinControlChange->text() : ""));
...
void SendCoinsDialog::coinControlChangeEdited(const QString & text)
{
    if (model)
    {
        model->coinControlChangeAddress(text);

        // label for the change address
        if (text.isEmpty())
            ui->labelCoinControlChangeLabel->setText("");
        else if (!CBitcoinAddress(text.toStdString()).IsValid())
            ui->labelCoinControlChangeLabel->setText(tr("Invalid Bitcoin address"));
        else
        {
...
void SendCoinsDialog::coinControlUpdateLabels()
{
    ...
    if (model->coinControlHasSelected())
    {
        // actual coin control calculation
        CoinControlDialog::updateLabels(model, this);

        // show coin control stats
        ui->frameCoinControl                        ->setMaximumHeight(170);
    ...
    else
    {
        ...
        ui->frameCoinControl                        ->setMaximumHeight(110);
