...
MONETARYUNITD=${1}/monetaryunitd
CLI=${1}/monetaryunit-cli
...
B4ARGS="-datadir=$D4"
$MONETARYUNITD $MONETARYUNITDARGS $B4ARGS &
B4PID=$!
...
function S {
  ...
  if [[ $TXID == "" ]] ; then
      echoerr "node${1}: error sending ${3} MUE"
...
function StartThree {
  $MONETARYUNITD $MONETARYUNITDARGS $B1ARGS &
  B1PID=$!
  $MONETARYUNITD $MONETARYUNITDARGS $B2ARGS &
  B2PID=$!
  $MONETARYUNITD $MONETARYUNITDARGS $B3ARGS &
  B3PID=$!
}
