...
# Add python-monetaryunitrpc to module search path:
import os
import sys
sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), "python-monetaryunitrpc"))
...
from monetaryunitrpc.authproxy import AuthServiceProxy, JSONRPCException
from util import *
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting MUE values"""
...
def initialize_chain(test_dir):
    ...
    if not os.path.isdir(os.path.join("cache", "node0")):
        ...
        for i in range(4):
            ...
            args = [ "monetaryunitd", "-keypool=1", "-datadir="+datadir ]
            if i > 0:
                args.append("-connect=127.0.0.1:"+str(START_P2P_PORT))
            monetaryunitd_processes.append(subprocess.Popen(args))
            subprocess.check_call([ "monetaryunit-cli", "-datadir="+datadir,
...
def start_nodes(num_nodes, dir):
    # Start monetaryunitds, and wait for RPC interface to be up and running:
    devnull = open("/dev/null", "w+")
    for i in range(num_nodes):
        datadir = os.path.join(dir, "node"+str(i))
        args = [ "monetaryunitd", "-datadir="+datadir ]
        monetaryunitd_processes.append(subprocess.Popen(args))
        subprocess.check_call([ "monetaryunit-cli", "-datadir="+datadir,
                                  "-rpcwait", "getblockcount"], stdout=devnull)
    devnull.close()
...
def wait_monetaryunitds():
    ...
    for monetaryunitd in monetaryunitd_processes:
        monetaryunitd.wait()
    del monetaryunitd_processes[:]
